...
const {Video} = require("../models/video"); // Correctly import Video and functions
...
router.post("/upload", authenticateJWT, upload.single("video"), async (req, res) => {
    try {
      ...
      // Send task message to SQS queue
      await sendMessage({
      ...
      console.log("Task message sent to SQS");
  
      res.status(200).json({ message: "File uploaded and task queued" });
    } catch (err) {
